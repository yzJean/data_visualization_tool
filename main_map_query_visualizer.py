...
Purpose: This tool aims at visualizing raw map and map handler query results.
Expected results: A figure containing the above information.

Command:
python3 main_map_query_visualizer.py -i <dir-to-map_*.csv> -u <dir-to-*data2verify.csv>

For example,
python3 main_map_query_visualizer.py -i map.csv -u data2verify.csv
"""
...
def options():
    ...
    p.add_option('--in_map_dir', '-i', default='./map.csv',
                 help='This is a mandatory option. It is recommended to use with the map.csv')
    p.add_option('--in_data_dir', '-u', default='',
                 help='This is an optional option. It is recommended to use with the ./*data2verify.csv')
    ...
    in_map_dir = options.in_map_dir
    in_data_dir = options.in_data_dir
    ...
    return in_map_dir, in_data_dir, display_modes_list, time_period, sampling_rate
...
def draw(filename, header_list, readings, display_modes_list, time_period, sampling_rate = 20):
    # ----- Read from map -----
    map_reading = readings[0]
    map_header_dict = header_list[0]
    info_list = parse_reading_dict_to_list(map_reading, map_header_dict)
    road_id_list = info_list[0]         # road_id
    map_lane_count_list = map_info_list[1]      # lane_count
    map_longitude_start_list = map_info_list[2] # longitude_start
    map_latitude_start_list = map_info_list[3]  # latitude_start
    map_longitude_end_list = map_info_list[4]   # longitude_end
    map_latitude_end_list = map_info_list[5]    # latitude_end

    # ----- Read from vehicle road data -----
    is_veh_road_data_imported = len(readings) >= 2
    # TODO: be more flexible
    if (is_veh_road_data_imported):
        data_reading = readings[1]
        data_header_dict = header_list[1]
        data_info_list = parse_reading_dict_to_list(data_reading, data_header_dict)
        # queried_passed
        veh_queried_passed = data_info_list[0]
        # ustt
        veh_ustt_list = data_info_list[1]
        # veh_longitude
        veh_longitude_list = data_info_list[2]
        # veh_latitude
        veh_latitude_list = data_info_list[3]
        # distance
        veh_distance_list = data_info_list[4]
        # matched road_index
        veh_lane_count_list = data_info_list[5] # TODO: road_id
        # matched road_index
        veh_road_index_list = data_info_list[6]
        # road_start_longitude
        veh_road_start_lon_list = data_info_list[7]
        # road_start_latitude
        veh_road_start_lat_list = data_info_list[8]
        # road_end_longitude
        veh_road_end_lon_list = data_info_list[9]
        # road_end_latitude
        veh_road_end_lat_list = data_info_list[10]
        # TODO
        is_advanced_dataset = len(data_info_list) > 12
        if (is_veh_road_data_imported and is_advanced_dataset):
            # veh_heading_deg
            veh_heading_deg_list = data_info_list[11]
            # closest_ptx
            veh_closest_pt_lat_list = data_info_list[12] #latitude
            # closest_pty
            veh_closest_pt_lon_list = data_info_list[13] #longitude
            # closest_ptz
            veh_closest_ptz_list = data_info_list[14]
            # veh2road_normalized
            # in terms of vehicle; positive: the road is on the right-hand sided of vehicle; otherwise, negative
            veh_veh2road_normalized_list = data_info_list[15]
        else:
        ...
        print(" Map data size:", len(map_road_id_list), ", veh_road_data size:", len(veh_longitude_list))
        ...
        if (is_veh_road_data_imported and is_advanced_dataset):
            ...
            print(" Map data size:", len(map_road_id_list), ", veh_road_data size:", len(tmp_veh_queried_passed))
    ...
    ax_subplot_map = painter.fig.add_subplot(111)
    ...
    # Draw SD map info
    ax_subplot_map.plot([map_longitude_start_list, map_longitude_end_list], [
                          map_latitude_start_list, map_latitude_end_list], color='k')

    for idex, item in enumerate(map_longitude_start_list):
        matched_road_v4 = [map_longitude_start_list[idex], map_latitude_start_list[idex],
                map_longitude_end_list[idex], map_latitude_end_list[idex]]
        painter.add_arrow(ax_subplot_map, matched_road_v4, 'k')

        # Draw whole map lane count information when veh_road_data is imported
        if (not is_veh_road_data_imported):
            lc_color = painter.get_color_with_index(idex)
            display_str_road_index = str(map_road_id_list[idex]) # to align with map.csv
            display_str_roadid = "rid" + display_str_road_index
            #TODO: lane count type
            if (is_display_lc):
                display_str_lc = (display_str_roadid + \
                    "_lc: 0") if (
                        type(map_lane_count_list[idex]) == "str") else display_str_roadid + "_lc: "+ str(map_lane_count_list[idex])
                painter.add_annotate(ax_subplot_map, display_str_lc, [
                                    map_longitude_start_list[idex], map_latitude_start_list[idex]], lc_color, idex)
        ...
        for idx, is_pass in enumerate(veh_queried_passed):
            ...
            if (is_draw and is_draw_in_time_period):
                ...
                if (is_draw_starter):
                    ...
                    painter.add_annotate(ax_subplot_map, display_str, [starter_x, starter_y], 'y')
                ...
                if (is_pass == 'pass'):
                    ...
                    ax_subplot_map.plot([veh_longitude_list[idx], veh_road_start_lon_list[queried_road_index]],
                                          [veh_latitude_list[idx], veh_road_start_lat_list[queried_road_index]],
                                          color=pose_color, linestyle='--')

                    # Draw matched road segment
                    ax_subplot_map.plot([veh_road_start_lon_list[queried_road_index], veh_road_end_lon_list[queried_road_index]],
                                          [veh_road_start_lat_list[queried_road_index], veh_road_end_lat_list[queried_road_index]],
                                          color=road_seg_color, linewidth=4)

                    # Draw map lane count information
                    display_str_road_id = str(
                        veh_road_index_list[queried_road_index])  # to align with map.csv
                    ...
                    if (is_veh_road_data_imported and is_advanced_dataset):
                        ...
                        ax_subplot_map.plot([veh_longitude_list[queried_road_index], veh_closest_pt_lon_list[queried_road_index]],
                    ...
                    if (is_display_frd_coord):
                        ...
                        # front
                        painter.add_heading(ax_subplot_map, pose_v2, 'salmon', heanding_len, heading)
                        # right
                        right_axis = heading - 90
                        painter.add_heading(ax_subplot_map, pose_v2, 'moccasin', heanding_len, right_axis)
                ...
                painter.add_text(ax_subplot_map, display_msg, [veh_longitude_list[idx], veh_latitude_list[idx]], pose_color)

    # Setup SDMap
    ax_subplot_info = {'xlabel': "Logitude",
                       'ylabel': "Latitude",
                       'title': "Map Matching Information: " + str(filename)}
    painter.setup_ax_subplot(ax_subplot_map, ax_subplot_info)
    painter.set_aspect(ax_subplot_map)

    # Setup distance
    ax_subplot_dist_info = {'xlabel': "Time (ustt)",
                       'ylabel': "Distance (meters)",
                       'title': "Map Distance: " + str(filename)}
    ...
    if (is_veh_road_data_imported and is_advanced_dataset):
        ax_subplot_heading_info = {'xlabel': "Time (ustt)",
                        ...
                        'title': "Map Heading: " + str(filename)}
...
# ========================
def main():
    in_map_dir, in_data_dir, display_modes_list, time_period, sampling_rate = options()
    map_filename = os.path.basename(in_map_dir)
    data_filename = os.path.basename(in_data_dir)
    display_filename = data_filename if data_filename != "" else map_filename

    map_header_dict = {'road_id': int,
                         'lane_count': str,
    ...
    data_header_dict = {'queried_passed': str, #0
                               'ustt': float,#1
    ...
    reading_directories = [in_map_dir, in_data_dir]
    header_list = [map_header_dict, data_header_dict]
